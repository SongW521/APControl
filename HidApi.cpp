...
int HidDevice::write(std::vector<unsigned char> data) const
{
    if( !this->isInitialized() )
    {
        if( this->deviceErrorCallback ) (*(this->deviceErrorCallback))(*this,HidError(HidError::DEVICE_INITIALIZED));
    ...
    std::vector<unsigned char> tempData = data;
    OVERLAPPED ol;
    ...
    if (!WriteFile(this->devHandle, &tempData[0], tempData.size(), NULL, &ol))
    {
        ...
        {
            bytesWritten = -1;
            if (this->deviceErrorCallback)
                (*(this->deviceErrorCallback))(*this, HidError(HidError::DEVICE_WRITE));
            return bytesWritten;
        }
    }
